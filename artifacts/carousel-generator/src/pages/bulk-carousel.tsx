...
export const BLOCK_STYLE: Record<BlockId, BlockStyle> = {
  ...
  body1: { font: '"Bebas Neue"', size: 50, lineH: 1.50, maxW: W - 160, label: "Body" },
  body2: { font: '"Bebas Neue"', size: 50, lineH: 1.50, maxW: W - 160, label: "Body" },
  body3: { font: '"Bebas Neue"', size: 50, lineH: 1.50, maxW: W - 160, label: "Body" },
  body4: { font: '"Bebas Neue"', size: 50, lineH: 1.50, maxW: W - 160, label: "Body" },
  body5: { font: '"Bebas Neue"', size: 50, lineH: 1.50, maxW: W - 160, label: "Body" },
  body6: { font: '"Bebas Neue"', size: 50, lineH: 1.50, maxW: W - 160, label: "Body" },
  body7: { font: '"Bebas Neue"', size: 50, lineH: 1.50, maxW: W - 160, label: "Body" },
  body8: { font: '"Bebas Neue"', size: 50, lineH: 1.50, maxW: W - 160, label: "Body" },
  body9: { font: '"Bebas Neue"', size: 50, lineH: 1.50, maxW: W - 160, label: "Body" },
  body10: { font: '"Bebas Neue"', size: 50, lineH: 1.50, maxW: W - 160, label: "Body" },
  cta:      { font: '"Bebas Neue"',     size:  76, lineH: 1.35, maxW: W - 140, label: "CTA"      },
...
async function openBankForMany(selectedItems: any[]) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const activeIndexes = csvRows.map((_, i) => i).filter((i) => rowSelected[i] !== false);
  ...
  const canGenerate = csvRows.length > 0 && selectedPresetId !== null && rowSelected.some((s) => s !== false);
                    ...
                    {csvRows.map((row, i) => (
                      <tr key={i} className="hover:bg-muted/10">
                        <td className="px-3 py-2 text-center">
                          <input
                            type="checkbox"
                            checked={rowSelected[i] !== false}
                            onChange={(e) => setRowSelected((prev) => { const next = [...prev]; next[i] = e.target.checked; return next; })}
                          />
                        </td>
          ...
          <div className="flex justify-end">
            <Button onClick={handleGenerate} disabled={rendering} size="lg" className="min-w-52">
              {rendering
                ...
                : `Generate ${rowSelected.filter((s) => s !== false).length} carousel${rowSelected.filter((s) => s !== false).length !== 1 ? "s" : ""}`}
            </Button>
          </div>
        )}
