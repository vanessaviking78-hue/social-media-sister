...
import { ArrowLeft, Upload, Download, Loader2, ChevronLeft, ChevronRight, Image as ImageIcon, CalendarClock, Music, Sparkles } from "lucide-react";
...
const [captionStyle, setCaptionStyle] = useState("2");
...
useEffect(() => { setCaptionOverride(""); }, [idx]);
...
if (i === idx && captionOverride.trim()) return captionOverride;
const sub = (subs[i] || subtitle || "").trim();
return quotes[i] + (sub ? `\n\n- ${sub}` : "");
};
...
const tid = toast.loading("Writing your caption…");
try {
const contextParts = [
`Quote: ${quotes[idx]}`,
subs[idx] ? `Attributed to: ${subs[idx]}` : "",
"End the caption with a warm, natural line encouraging whoever's reading to share this post or tag someone who needs to see it, whichever fits better. Keep it low pressure, like a genuine ask, not a demand.",
];
const context = contextParts.filter(Boolean).join("\n");
const r = await fetch(`${BASE}api/caption-generator/generate`, {
method: "POST",
headers: { "Content-Type": "application/json" },
body: JSON.stringify({ tone: captionStyle, context, clinicName: clientName || undefined }),
});
const d = await r.json();
if (!r.ok || !d.caption) throw new Error(d.error || "Caption generation failed");
setCaptionOverride(d.caption);
toast.success("Caption ready.", { id: tid });
} catch (e: unknown) {
toast.error(e instanceof Error ? e.message : "Caption generation failed", { id: tid });
...
<section className="space-y-2">
<h2 className="font-semibold text-base">4. Caption</h2>
<p className="text-xs text-muted-foreground">By default the caption is just the quote and who it's from. Generate a proper caption instead, with a natural nudge to share or tag someone, in one of your styles.</p>
<div className="flex gap-2">
<select value={captionStyle} onChange={(e) => setCaptionStyle(e.target.value)} className="rounded-lg bg-muted/30 border border-border/30 text-sm px-2 py-2 focus:outline-none focus:ring-1 focus:ring-primary/50">
{CAPTION_STYLES.map((s) => <option key={s.value} value={s.value}>{s.label}</option>)}
</select>
<Button variant="outline" onClick={generateCaption} disabled={generatingCaption || !quotes.length} className="flex-1">
<Sparkles className="w-4 h-4 mr-1.5" />{generatingCaption ? "Writing…" : "Generate Caption"}
</Button>
</div>
<textarea value={captionOverride} onChange={(e) => setCaptionOverride(e.target.value)} placeholder="Leave blank to use the quote itself as the caption" rows={3} className="w-full rounded-lg bg-muted/30 border border-border/30 text-sm px-3 py-2 resize-y focus:outline-none focus:ring-1 focus:ring-primary/50" />
</section>
</div>
