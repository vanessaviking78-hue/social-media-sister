...
type ReportFull = ReportListItem & {import { useEffect, useState } from "react";
import { toast } from "sonner";
import { Search, Trash2, ChevronLeft, Download, Loader2 } from "lucide-react";
...
export default function CompetitorScout() {
  ...
  const DOWNLOAD_STYLES = `
    ...
    h1{font-family:'Anton',Arial,sans-serif;font-weight:400;font-size:2.75rem;line-height:1.05;letter-spacing:0.01em;color:#ec1a99;margin:0 0 6px;}
    .cs-doc-meta{color:#b3b0c4;font-size:0.85rem;margin-bottom:32px;}
    h2{font-family:Arial,sans-serif;font-size:0.75rem;text-transform:uppercase;letter-spacing:0.06em;color:#ec1a99;font-weight:600;margin:32px 0 8px;}
  ...
  // Older reports were generated with the AI writing its own "where I'd
  // focus your socials next" ideas list and closing pitch. That's now a
  // fixed line Vanessa writes herself rather than something the AI
  // generates fresh each time, so this cuts off anything from that heading
  // onward (on older reports it'll still be there in report_html) and
  // always appends the same fixed suggestions box instead. New reports
  // never have that heading in the first place, so this is a no-op for
  // them beyond adding the box.
  function finalizeReportHtml(html: string): string {
    const marker = "<h2>Where I'd focus your socials next</h2>";
    const idx = html.indexOf(marker);
    const trimmed = (idx !== -1 ? html.slice(0, idx) : html).trimEnd();
    const suggestions = `<div class="cs-cta"><p><strong>Suggestions</strong> - film 5 reels a week, see Vanessa for titles and scripts. Content days with professional images, videos and podcast episode would really help personify the page, see Vanessa for current deals.</p></div>`;
...
${finalizeReportHtml(report.report_html)}
</body></html>`;
      ...
      <div className="min-h-[100dvh] w-full bg-background text-foreground">
        <header className="border-b border-border/40 px-6 py-5 flex items-center gap-3">
          ...
          <div className="flex-1">
            <h1 className="text-3xl sm:text-4xl leading-none text-[#ec1a99]" style={{ fontFamily: "'Anton', sans-serif" }}>{open.clinic_name}</h1>
            <p className="text-sm text-muted-foreground mt-1">{open.postcode} · {new Date(open.created_at).toLocaleDateString("en-GB")}</p>
          </div>
          <button
        ...
        <main className="max-w-3xl mx-auto px-6 py-8">
          <div
            className="cs-report prose prose-invert max-w-none prose-h2:text-xs prose-h2:uppercase prose-h2:tracking-wide prose-h2:text-[#ec1a99] prose-h2:font-semibold prose-h2:mt-8 prose-h2:mb-2"
            dangerouslySetInnerHTML={{ __html: finalizeReportHtml(open.report_html) }}
          />
        </main>
      </div>
    );
