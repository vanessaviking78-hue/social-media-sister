    ...
    <div className="min-h-screen bg-zinc-950 text-white">
      ...
      {showTip && (
        <div className="max-w-3xl mx-auto px-4 pt-4">
          <div className="rounded-xl border border-pink-800/40 bg-pink-950/20 px-4 py-2.5 flex items-center gap-2 text-xs text-pink-200">
            <Share className="w-4 h-4 shrink-0" />
            <span>Tip: tap your browser's Share button, then <b>Add to Home Screen</b>, to keep your portal one tap away.</span>
            <button onClick={() => setShowTip(false)} className="ml-auto text-pink-400/70 hover:text-pink-300">Got it</button>
          </div>
        </div>
      )}
              ...
              <div className="space-y-3">
                {data.upcomingPosts.map((post) => (
                  <div key={post.id} className="rounded-2xl border border-zinc-800 bg-zinc-900/60 overflow-hidden flex">
                    {post.imageUrl ? (
                      <div className="w-20 shrink-0 bg-zinc-800"><img src={post.imageUrl} alt="" className="w-full h-full object-cover" style={{ minHeight: 80 }} /></div>
                    ) : (
                      <div className="w-20 shrink-0 flex items-center justify-center" style={{ backgroundColor: post.color + "22", borderRight: `2px solid ${post.color}44` }}><FileImage className="w-5 h-5 text-zinc-600" /></div>
                    )}
                    <div className="flex-1 px-4 py-3 min-w-0">
                      ...
                      {post.caption && <p className="text-xs text-zinc-500 mt-0.5 line-clamp-2">{post.caption}</p>}
                    </div>
                  </div>
                ))}
              </div>
          ...
          <section>
            ...
            {pendingBatches.length === 0 && reviewedBatches.length === 0 && (
              <div className="rounded-2xl border border-zinc-800 bg-zinc-900/40 p-8 text-center">
                ...
                <p className="text-zinc-500">Nothing waiting on you right now.</p>
              </div>
            )}
                ...
                {pendingBatches.map((b) => (
                  <a key={b.id} href={`${BASE}approve/${b.token}`} className="flex items-center gap-3 rounded-2xl border border-pink-800/40 bg-pink-950/10 hover:bg-pink-950/20 transition-colors px-4 py-4">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-xs text-zinc-400">{b.pendingImages} waiting &middot; {b.approvedImages} approved</p>
                    </div>
                    <span className="text-xs font-semibold text-pink-300 flex items-center gap-1">Review <ChevronRight className="w-4 h-4" /></span>
                  </a>
                ))}
              </div>
            )}
            {reviewedBatches.length > 0 && (
              <div className="space-y-3">
                ...
                {reviewedBatches.map((b) => (
                  <div key={b.id} className="flex items-center gap-3 rounded-2xl border border-zinc-800 bg-zinc-900/40 px-4 py-4">
                    <CheckCircle2 className="w-5 h-5 text-green-500 shrink-0" />
                    <div className="min-w-0 flex-1">
                      <p className="font-medium text-white text-sm truncate">{b.name}</p>
                      <p className="text-xs text-zinc-500">{b.approvedImages} approved &middot; {b.rejectedImages} flagged</p>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="space-y-5">
                <div className="grid grid-cols-2 gap-3">
                  {(["before", "after"] as const).map((which) => {
                    const prev = which === "before" ? beforePrev : afterPrev;
