...
router.get("/analytics/summary", async (_req, res) => {
  try {
    const [totals] = await db.select({
      ...
      totalBeforeAfters: sql<number>`coalesce(sum(${activityLogTable.postCount}) filter (where ${activityLogTable.action} = 'generated' and ${activityLogTable.postType} = 'before-after'), 0)::int`,
