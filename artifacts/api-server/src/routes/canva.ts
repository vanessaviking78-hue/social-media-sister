...
// FIX: Canva Connect API uses /rest/v1/asset-uploads (async job) not /rest/v1/assets (direct).
// The old direct endpoint rejects image/png. The asset-uploads endpoint handles all image types.
...
let assetId = canvaData.asset?.id;
...
if (!pollRes.ok) break;
const pollData = (await pollRes.json()) as { job?: { status: string; asset?: { id: string } } };
if (pollData.job?.status === "success") { assetId = pollData.job.asset?.id; break; }
if (pollData.job?.status === "failed") throw new Error("Canva asset upload job failed");
}
}
