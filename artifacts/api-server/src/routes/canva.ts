...
router.get("/canva/auth/start", (req: Request, res: Response) => {
  try {
    ...
    const params = new URLSearchParams({
      ...
      // profile:read added so we can capture the connected user's display name on callback
      scope: "asset:read asset:write profile:read",
      state,
...
router.post("/canva/upload", async (req: Request, res: Response) => {
  try {
    ...
    // Canva's max asset name is 50 characters (unencoded) - anything longer gets rejected.
    const assetName = (name ?? "Social Media Sister Export").slice(0, 50);
    const metadata = JSON.stringify({ name_base64: Buffer.from(assetName).toString("base64") });

    // FIX: the Canva Connect API requires Content-Type: application/octet-stream on this
    // endpoint regardless of the actual image type - sending the real mime type (image/png,
    // image/jpeg etc) causes Canva to reject the request. This was the cause of every
    // "Canva upload failed" error.
    const canvaRes = await fetch("https://api.canva.com/rest/v1/asset-uploads", {
      method: "POST",
      headers: {
    ...
    const canvaData = (await canvaRes.json()) as {
      job?: { id: string; status: string; asset?: { id: string }; error?: { code?: string; message?: string } };
      asset?: { id: string };
      code?: string;
      message?: string;
    };
    ...
    if (!canvaRes.ok) {
      throw new Error(canvaData.message ?? canvaData.code ?? canvaData.job?.error?.message ?? "Canva upload failed");
    }

    if (canvaData.job?.status === "failed") {
      throw new Error(canvaData.job.error?.message ?? "Canva asset upload job failed");
    }

    // If the API returned an async job, poll until it finishes (usually under 10s)
    let assetId = canvaData.asset?.id ?? canvaData.job?.asset?.id;
    if (!assetId && canvaData.job?.id) {
      const jobId = canvaData.job.id;
      for (let i = 0; i < 30; i++) {
        ...
        const pollData = (await pollRes.json()) as {
          job?: { status: string; asset?: { id: string }; error?: { code?: string; message?: string } };
        };
        if (pollData.job?.status === "success") { assetId = pollData.job.asset?.id; break; }
        if (pollData.job?.status === "failed") {
          throw new Error(pollData.job.error?.message ?? "Canva asset upload job failed");
        }
      }
    }

    req.log.info({ assetId, assetName }, "canva asset uploaded");
