...
export const HOMEWORK_SHOTS_PRESETS: PhotoStudioPreset[] = [
  {
    ...
    hasColour: true,
    promptTemplate: `IMPORTANT INSTRUCTION: use the reference photo ONLY to copy her exact face, hair colour and style, and skin tone. Ignore and discard every other element of the reference photo — do not reuse its background, its navy or blue tones, its lighting, or its clothing. She must be photographed standing directly in front of a plain, flat, solid [COLOUR] wall — not a studio, not a backdrop stand, not a photography set, just an ordinary flat wall painted a single solid [COLOUR] colour with no texture, no gradient, no shadows, and no props. There must be zero navy, zero blue, and zero trace of the original setting left in this image — only the plain solid [COLOUR] wall filling the entire background behind her.\n\nShe is wearing [SCRUB COLOUR] medical scrubs, cropped from the waist up only with no legs or lower body visible in the shot, shot like a professional but natural phone photo for social media content, both index fingers extended and pointing straight down toward the bottom of the frame as if directing the viewer to read a caption below, relaxed confident expression. Hyper-realistic unretouched skin texture with visible pores, no airbrushing, no smoothing, no glamming up. Same facial features as reference photo.`,
  },
  {
    id: "hw-02",
