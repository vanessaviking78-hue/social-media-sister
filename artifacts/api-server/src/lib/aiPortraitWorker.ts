...
import { buildPrompt, buildCustomPrompt, buildPhotoStudioPrompt, AI_PORTRAIT_SCENARIOS, PHOTO_STUDIO_PRESETS, INJECTOR_COLLECTION_PRESETS, MEN_STUDIO_PRESETS, RANDOM_PROMPT_PRESETS } from "./aiPortraitScenarios";
...
export async function processPortraitJob(
  ...
  for (let i = 0; i < scenarios.length; i++) {
    ...
    const photoStudioPreset = PHOTO_STUDIO_PRESETS.find((p) => p.id === cfg.id) ?? INJECTOR_COLLECTION_PRESETS.find((p) => p.id === cfg.id) ?? MEN_STUDIO_PRESETS.find((p) => p.id === cfg.id) ?? RANDOM_PROMPT_PRESETS.find((p) => p.id === cfg.id);
