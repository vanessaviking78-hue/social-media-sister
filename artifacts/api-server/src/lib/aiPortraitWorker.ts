...
export async function processPortraitJob(
  ...
  for (let i = 0; i < scenarios.length; i++) {
    ...
    let photoStudioPreset;
    try {
      photoStudioPreset =
        NEW_PORTRAITS_PRESETS.find((p) => p.id === cfg.id) ??
        JULY_2ND_SHOOT_PRESETS.find((p) => p.id === cfg.id) ??
      ...
      logger.error({ jobId, scenarioId: cfg.id, err: msg }, "Preset lookup crashed before generation started");
      patchCard({ status: "failed", failureReason: `Preset lookup error: ${msg}` });
      continue;
    }
